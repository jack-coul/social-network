  ...
  const handleSendMess = (id1) => {
    for (let j = 0; j < conversations.length; j++) {
      for (let i = 0; i < conversations[j].members.length; i++) {
        if (
          conversations[j].members[i]._id === id1 ||
          conversations[j].members[i]._id === id
        )
          if (
            conversations[j].members[i + 1]?._id === id ||
            conversations[j].members[i + 1]?._id === id1
          ) {
            return true;
          }
      }
